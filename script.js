...
function showQuestion() {
  ...
  // Add Second Chance button dynamically if not already added
  if (!document.getElementById("secondChanceBtn")) {
    const secondBtn = document.createElement("button");
    secondBtn.id = "secondChanceBtn";
    secondBtn.textContent = "❤️ Second Chance";
    secondBtn.onclick = useSecondChance;
    document.getElementById("lifelines").appendChild(secondBtn);
  }
...
function useSecondChance() {
  ...
  document.getElementById("secondChanceBtn").classList.add("used");
  alert("Second Chance activated! Wrong answer will be forgiven once.");
}
...
function checkAnswer(ans) {
  ...
  buttons.forEach(b => {
    b.disabled = true;
    if (b.textContent === correct) b.classList.add("correct", "glow");
    if (b.textContent === ans && ans !== correct) b.classList.add("wrong", "shake");
  });
...
function showFinalScreen() {
  stopAllSounds();
  playSound("win");
  const guaranteed = Math.floor(ladderLevel / 5) * 500;

  quizDiv.innerHTML = `
    <div class="final-screen">
      <h1>🎉 CONGRATULATIONS</h1>
      <h2>You Won $${ladderLevel * 100} (Guaranteed: $${guaranteed})</h2>
...
async function saveScore(score) {
  ...
  if (doc.exists) {
    const total = (doc.data().score || 0) + score;
    await userRef.set({ name: user.displayName || "Guest", photo: user.photoURL || "", score: total });
  } else {
    await userRef.set({ name: user.displayName || "Guest", photo: user.photoURL || "", score });
  }
}
