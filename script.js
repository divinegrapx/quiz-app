...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const firebaseConfig = {
    apiKey: "AIzaSyBS-8TWRkUlpB36YTYpEMiW51WU6AGgtrY", // <- replace with your valid Firebase API key
  ...
  // create hintBox dynamically if not present
  ...
  function buildMoneyLadder() {
    ...
    // generate dynamic ladder based on number of questions
    const step = Math.ceil(moneyLevels.length / count);
    for (let i = 0; i < count; i++) {
      const li = document.createElement("li");
      li.textContent = moneyLevels[i * step] || moneyLevels[moneyLevels.length - 1];
