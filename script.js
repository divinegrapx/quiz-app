...
onAuthStateChanged(auth, user => {
  if (user) {
    ...
    startQuiz(); // start automatically after login
  } else {
    authDiv.style.display = "block";
    emailDiv.style.display = "none";
...
async function startQuiz() {
  current = 0;
  score = 0;
  buildMoneyLadder(10); // default 10 questions
...
function checkAnswer(btn, correct) {
  ...
  document.querySelectorAll(".option-btn").forEach(b => {
    b.disabled = true;
    if (b.textContent === questions[current].correctAnswer) b.classList.add("correct");
  });
...
// ================= MONEY LADDER =================
function buildMoneyLadder(total) {
  moneyList.innerHTML = "";
...
callBtn.onclick = () => {
  ...
  lifelineResult.textContent = success
    ? `📞 Friend thinks the answer is: "${correct}"`
    : `📞 Friend is unsure… maybe "${pickWrong()}"`;
...
audienceBtn.onclick = () => {
  ...
  const votes = {};
  options.forEach(o => votes[o] = Math.floor(Math.random() * 20));
  votes[correct] += 40 + Math.floor(Math.random() * 20);
  lifelineResult.innerHTML = Object.entries(votes)
    .map(([k, v]) => `${k}: ${v}%`)
    .join("<br>");
...
function pickWrong() {
  return questions[current].incorrectAnswers[
    Math.floor(Math.random() * questions[current].incorrectAnswers.length)
  ];
}
